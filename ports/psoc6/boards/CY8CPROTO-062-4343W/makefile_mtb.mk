...
MYP_MTB_MAKEFILE="$(MYP_DIR_OF_THIS_MAKEFILE)/Makefile"
...
mtb:
	...
	-$(Q) cd boards/$(MYP_MTB_BOARD_NAME) ; make build
...
myp_define_mtb_vars: mtb
	$(eval MYP_MTB_INCLUDE_DIRS = $(file < $(MYP_DIR_OF_THIS_MAKEFILE)/build/$(MYP_MTB_TARGET)/$(MYP_MTB_CONFIG)/inclist.rsp))
	$(info MYP_MTB_INCLUDE_DIRS : $(MYP_MTB_INCLUDE_DIRS))
	$(info )
	$(eval INC += $(subst -I,-Iboards/$(MYP_MTB_BOARD_NAME)/,$(MYP_MTB_INCLUDE_DIRS)))
	$(info )
	$(eval MYP_MTB_ALL_OBJECTS = $(file < $(MYP_DIR_OF_THIS_MAKEFILE)/build/$(MYP_MTB_TARGET)/$(MYP_MTB_CONFIG)/objlist.rsp))
	$(eval MYP_MTB_OBJECTS = $(filter-out %/main.o,$(MYP_MTB_ALL_OBJECTS)))
	$(info MYP_MTB_OBJECTS : $(MYP_MTB_OBJECTS))
	$(info )
	$(eval OBJ += $(MYP_MTB_OBJECTS))
	$(info )
	$(eval MYP_MTB_LIBRARIES=$(file < $(MYP_DIR_OF_THIS_MAKEFILE)/build/$(MYP_MTB_TARGET)/$(MYP_MTB_CONFIG)/liblist.rsp))
	...
	$(info )
	$(eval LDFLAGS += -mcpu=cortex-m4 --specs=nano.specs -mfloat-abi=softfp -mfpu=fpv4-sp-d16 -mthumb -ffunction-sections -fdata-sections -ffat-lto-objects -g -Wall -pipe -Wl,--gc-sections -T$(MYP_DIR_OF_THIS_MAKEFILE)/bsps/TARGET_APP_CY8CPROTO-062-4343W/COMPONENT_CM4/TOOLCHAIN_GCC_ARM/linker.ld -Wl,-Map,$(MYP_DIR_OF_THIS_MAKEFILE)/build/firmware.map $(MYP_MTB_OBJECTS) -Wl,--start-group -Wl,--end-group )
	$(info )
	$(info LD : $(LD))
	$(info )
